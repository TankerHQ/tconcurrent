...
{
  auto sender = lazy::run_resumable(get_default_executor(),
                                    []() -> cotask<int> { TC_RETURN(42); });
...
TEST_CASE("coroutine await lazy sender")
{
  ...
  auto f = async_resumable(
      [sender]() -> cotask<int> { TC_RETURN(TC_AWAIT(sender)); });
  CHECK(42 == f.get());
}
...
TEST_CASE("coroutine await lazy sender void")
{
  ...
  auto f = async_resumable([sender]() -> cotask<void> { TC_AWAIT(sender); });
  CHECK_NOTHROW(f.get());
}
