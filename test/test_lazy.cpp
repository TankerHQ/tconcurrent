...
TEST_CASE("lazy sync_wait with value")
{
  ...
  auto const val = lazy::sync_wait<int>(
...
TEST_CASE("lazy sync_wait with error")
{
  lazy::cancelation_token c;
  CHECK_THROWS_AS(
      lazy::sync_wait<int>(make_sender<int>([](auto&& receiver) {
...
{
  lazy::cancelation_token c;
  lazy::sync_wait<move_only>(make_sender<move_only>([](auto&& receiver) {
                               receiver.set_value(move_only{});
                             }),
                             c);
}
...
TEST_CASE("lazy then returning value")
{
  ...
  auto const result = lazy::sync_wait<int>(then, c);
  CHECK(result == 42);
}
...
TEST_CASE("lazy then receiving and returning void")
{
  ...
  auto const s2 = []() {};
  lazy::sync_wait<void>(lazy::then(s1, s2), c);
}
...
TEST_CASE("lazy then receiving and returning move only type")
{
  ...
  lazy::sync_wait<move_only>(lazy::then(s1, s2), c);
}
...
TEST_CASE("lazy then with error")
{
  ...
  CHECK_THROWS_AS(lazy::sync_wait<int>(lazy::then(s1, twice), c), int);
}
...
TEST_CASE("lazy then never run because of abort")
{
  ...
  CHECK_THROWS_AS(lazy::sync_wait<int>(lazy::then(s1, twice), c),
                  operation_canceled);
}
...
TEST_CASE("lazy async_then returning value")
{
  ...
  auto const result = lazy::sync_wait<int>(lazy::async_then<int>(s1, twice), c);
  CHECK(result == 42);
}
...
TEST_CASE("lazy async_then receiving and returning void")
{
  ...
  lazy::sync_wait<void>(lazy::async_then<>(s1, s2), c);
}
...
TEST_CASE("lazy async_then receiving and returning move only type")
{
  ...
  lazy::sync_wait<move_only>(lazy::async_then(s1, s2), c);
}
...
TEST_CASE("lazy async_then with error")
{
  ...
  CHECK_THROWS_AS(lazy::sync_wait<int>(lazy::async_then(s1, twice), c), int);
}
...
TEST_CASE("lazy async_then never run because of abort")
{
  ...
  auto const s1 =
      make_sender<int>([](auto&& receiver) { receiver.set_done(); });
  auto const twice = [](auto r, int v) { REQUIRE(false); };
  CHECK_THROWS_AS(lazy::sync_wait<int>(lazy::async_then(s1, twice), c),
                  operation_canceled);
}
...
TEST_CASE("canceler is called if cancel is already requested")
{
  ...
  CHECK_THROWS_AS(lazy::sync_wait<void>(sender, c), operation_canceled);
}
...
TEST_CASE("then does not run continuation if cancel was requested")
{
  ...
  CHECK_THROWS_AS(lazy::sync_wait<void>(lazy::then(s1, s2), c),
                  operation_canceled);
}
...
TEST_CASE("async_then does not run continuation if cancel was requested")
{
  ...
  CHECK_THROWS_AS(lazy::sync_wait<void>(lazy::async_then(s1, s2), c),
                  operation_canceled);
}
...
TEST_CASE("then does not run an obsolete canceler")
{
  ...
  CHECK_NOTHROW(lazy::sync_wait<void>(lazy::then(s1, s2), c));
}
...
TEST_CASE("async_then does not run an obsolete canceler")
{
  ...
  CHECK_NOTHROW(lazy::sync_wait<void>(lazy::async_then(s1, s2), c));
}
