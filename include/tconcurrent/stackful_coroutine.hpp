...
struct extract_tuple_one
{
  static_assert(!sizeof(Tuple), "can't use await on a multiple-value sender");
};

template <typename Value>
struct extract_tuple_one<std::tuple<Value>>
{
  using type = Value;
};

template <>
struct extract_tuple_one<std::tuple<>>
{
  using type = void;
};
        ...
        typename std::decay_t<Sender>::template value_types<std::tuple>>**)
{
  ...
  using return_type = typename extract_tuple_one<
      typename Sender::template value_types<std::tuple>>::type;
...
template <typename E, typename F>
auto run_resumable(E&& executor, F&& cb, std::string name = {})
{
  return detail::run_resumable_sender<std::decay_t<E>, std::decay_t<F>>{
      std::forward<E>(executor), std::forward<F>(cb), std::move(name)};
}
}
...
auto async_resumable(std::string const& name, E&& executor, F&& cb)
{
  ...
  auto task = lazy::connect(
      ...
      lazy::run_resumable(executor, std::forward<F>(cb), fullName));
