...
struct async_sender
{
  ...
  {
    auto data = std::make_shared<detail::async_data<std::decay_t<R>>>(
        std::forward<R>(receiver));
    data->receiver.get_cancelation_token()->set_canceler([data]() mutable {
...
/** Make a sender that will call its receiver on \p executor.
 */
template <typename E>
